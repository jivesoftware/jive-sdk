...
exports.prepare = function (rootDir, tilesDir, appsDir, cartridgesDir, storagesDir, packageApps) {
    ...
    var extensionServerDir = extensionSrcDir + '/server';
    ...
    return jive.util.fsexists(extensionSrcDir).then(function (exists) {
        if (!exists) {
            jive.util.fsmkdir(extensionSrcDir)
        }
    }).then(function () {
        if (packageApps) {
        ...
        if (packageApps) {
            ...
            return jive.util.fsexists(extensionServerDir).then(function (exists) {
                return !exists ? jive.util.fsmkdir(extensionServerDir) : q.resolve();
            }).then(function () {
                fs.readdir(".", function (err, files) {
                    ...
                    files.filter(function (file) {
                        return file !== "extension.zip" && file !== "extension_src" && file !== "node_modules";
                    }).forEach(function (file) {
                        console.log(file);
                        copyRecursiveSync(file, extensionServerDir + "/" + file);
...
function fillExtensionMetadata(extensionInfo, definitions, packageApps, cartridges) {
    ...
    // suppress keys stuff that shouldn't be in meta
    delete extensionMeta['uuid'];
    delete extensionMeta['jiveServiceSignature'];
    if ( !extensionMeta['global_update_mac'] ) {
        // if empty key, delete
        delete extensionMeta['global_update_mac'];
    }
...
function setupExtensionDefinitionJson(tilesDir, appsDir, cartridgesDir, storagesDir, extensionSrcDir,
                                      extensionPublicDir,
        ...
        return getApps(appsDir, extensionPublicDir, extensionInfo, packageApps).then(function (apps) {
            return getStorages(storagesDir, extensionInfo).then(function (storages) {
                return getCartridges(cartridgesDir, extensionSrcDir).then(function (cartridges) {
                    ...
                    return jive.util.fswrite(stringifiedDefinitionJson,
                            definitionJsonPath).then(function () {
                ...
                dirContents.forEach(function (item) {
                    ...
                    if (isDirectory(appDir)) {
                        ...
                        var copyAppDirToPublic = function (app) {
                            return packageApps ? jive.util.fscopy(appDir + '/public',
                                    extensionPublicDir + '/apps/' + item).then(function () {
                        ...
                        var replacePublicReferences = function (target) {
                            ...
                            jive.util.fsread(extensionPublicDir + '/' + t).then(function (data) {
                                ...
                                jive.util.fswrite(processed,
                                        extensionPublicDir + '/' + t).then(function () {
                                        deferred.resolve();
                                    });
                            });
                        ...
                        var sanitizeAppPublicDirReferences = function (app) {
                            ...
                            return jive.util.recursiveDirectoryProcessor(root, root, '/tmp', true,
                                function (type, currentFsItem) {
                                    return q.fcall(function () {
                                        var proms = [];
                                        var lowercased = currentFsItem.toLowerCase();
                                        var ext = lowercased.substring(currentFsItem.lastIndexOf('.') + 1,
                                            lowercased.length);
                                        var validType = ext === 'xml' || ext === 'html' || ext === 'css' ||
                                            ext === 'js';
                                        if (type === 'file' && validType) {
                                            var target = currentFsItem.substring(currentFsItem.indexOf('/') +
                                                1, currentFsItem.length);
            ...
            if (!exists) {
                ...
                return jive.service.extensions().find().then(function (extension) {
                    return jive.util.fsexists(templatesPath.trim()).then(function (exists) {
                        if (!exists) {
                            // create it
                            return buildDefaultTemplate(allDefinitions,
                                extension).then(function (defaultTemplate) {
                                    var templates = {};
                                    templates['default'] = defaultTemplate;
                                    return jive.util.fswrite(JSON.stringify(templates, null, 4),
                                            tilesDir + '/templates.json').then(function () {
