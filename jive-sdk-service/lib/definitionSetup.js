        ...
        function(app, definitionName, theFile, theDirectory) {
            ...
            if ( ( service.role.isWorker() || service.role.isPusher() ) && tasks) {
                ...
                tasksToAdd.forEach(function(task) {
                    ...
                    var interval = task['interval'] || 60 * 1000; //amount of time between executions of the task
                    var timeout = task['timeout'] || 30 * 1000; //timeout is the amount of time to wait on the job before we assume its worker crashed
