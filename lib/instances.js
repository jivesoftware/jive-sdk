...
exports.register = function (jiveUrl, pushUrl, config, name, code ) {
    ...
    var clientSecret = jive.service.options['clientSecret'];
    ...
    if ( jiveUrl ) {
        // we're interacting directly with a jive instance (eg. > Samurai)
        // look up the clientID/secret for that in the trust store
        jive.service.persistence().find('jiveTrustStore', { 'jiveUrl' : jiveUrl } ).then(
            function(found) {
                if ( found && found.length > 0 ) {
                    options['client_id'] = found['clientId'] || clientId;
                    options['client_secret'] = found['clientSecret'] || clientSecret;
            ...
            function(error) {
                // could not look it up... for some reason
                // default to registration using global params
                doRegister(options, pushUrl, config, name, jiveUrl, deferred );
            }
        );
    } else {
