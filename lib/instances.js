...
var q = require('q');
var dispatcher = require('./persistence/dispatcher');
...
Instances.prototype.persistence = function() {
    return dispatcher;
};
...
Instances.prototype.register = function (client_id, url, config, name, code ) {
    ...
    jiveClient.requestAccessToken(options,
        ...
        // success
        function (response) {
            console.log("Reached Access Token Exchange callback", response);
        ...
        // failure
        function(err) {
            console.log("Failed access token exchange!", err);
            deferred.reject( err );
        }
    );
