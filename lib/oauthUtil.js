var url = require('url');
var util = require('util');
var jive = require('../api');

exports.getRedirectUri = function() {
    var clientUrl = jive.service.options['clientUrl'] + ':' + jive.service.options['port'];
    return process.env['jive_oauth2_redirect']
        || (jive.service.options['oauth2'] ? jive.service.options['oauth2']['clientOAuth2CallbackUrl'] : undefined)
        || (clientUrl + '/oauth2Callback');
};

exports.buildAuthorizeUrlResponseMap = function( oauth2Conf, callback, context, extraAuthParams ) {
    var stateToEncode = { 'jiveRedirectUrl' : callback };
    if ( context ) {
        stateToEncode = util._extend(stateToEncode, context);
    }

    var clientUrl = jive.service.options['clientUrl'] + ':' + jive.service.options['port'];
    var redirectUrl = exports.getRedirectUri();

    var oauth2AuthorizePath = oauth2Conf['oauth2AuthorizePath'];
    var oauth2ConsumerKey = oauth2Conf['oauth2ConsumerKey'];
    var originServerAuthorizationUrl = oauth2Conf['originServerAuthorizationUrl'];

    var url  = originServerAuthorizationUrl + "?" +
        "state=" + jive.util.base64Encode(JSON.stringify(stateToEncode)) +
        "&redirect_uri=" + redirectUrl +
        "&client_id=" + oauth2ConsumerKey +
        "&response_type=code";

    if ( extraAuthParams ) {
        var extraAuthStr = '';
        for (var key in extraAuthParams) {
            if (extraAuthParams.hasOwnProperty(key)) {
                extraAuthStr += '&';
                extraAuthStr += key + '=' + extraAuthParams[key];

            }
        }

        url += extraAuthStr;
    }

    return {
        'url' : url
    };
};

exports.buildOauth2CallbackObject = function(oauth2Conf, code, extraParams ) {

    var redirectUrl = exports.getRedirectUri();

    var postObject = {
        'grant_type'        : 'authorization_code',
        'client_id'         : oauth2Conf['oauth2ConsumerKey'],
        'client_secret'     : oauth2Conf['oauth2ConsumerSecret'],
        'redirect_uri'      : redirectUrl,
        'code'              : code
    };

    if ( extraParams ) {
        postObject = util._extend(postObject, extraParams);
    }

    return postObject;
};

